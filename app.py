...
def simpan_jadual_db(bulan, tahun, df_jadual):
    ...
    # 1. Padam rekod sedia ada bagi bulan dan tahun tersebut
    c.execute("DELETE FROM rekod_jadual WHERE bulan = ? AND tahun = ?", (bulan, tahun))
    conn.commit()
    
    # 2. Sediakan DataFrame untuk disimpan
    ...
    # Pastikan nilai lalai bagi bahagian jika tiada
    if 'bhg_pegawai' not in df_to_save.columns:
        df_to_save['bhg_pegawai'] = 'E8'
    if 'bhg_penolong' not in df_to_save.columns:
        df_to_save['bhg_penolong'] = 'E2'
    ...
    # 3. Simpan menggunakan Pandas to_sql (Memutus masalah mismatched placeholder)
...
def jana_docx_lampiran_a(df_jadual, bulan, tahun):
    ...
    for idx, row in df_jadual.iterrows():
        ...
        # 2. Tkh / Masa
        masa = "1700-0800"
        if "Siang" in str(row['Syif / Masa']): masa = "0800-2000"
        elif "Malam" in str(row['Syif / Masa']): masa = "2000-0800"
        row_cells[1].text = f"{row['Tarikh']}\n{masa}"
        row_cells[1].paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.CENTER
        
        # 3. Hari
        row_cells[2].text = str(row['Hari']).upper()
        row_cells[2].paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.CENTER
        
        # 4. Nama Pegawai & Penolong
        row_cells[3].text = f"{row['Nama Pegawai']}\n{row['Nama Penolong']}"
        
        # 5. No Telefon
        tel_peg = str(row.get('No. Tel Pegawai', '')).strip()
        tel_pen = str(row.get('No. Tel Penolong', '')).strip()
        row_cells[4].text = f"({tel_peg})\n({tel_pen})"
        row_cells[4].paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.CENTER
        
        # 6. Bahagian
        bhg_peg = row.get('Bhg Pegawai', 'E8')
        bhg_pen = row.get('Bhg Penolong', 'E2')
    ...
    # Simpan ke Buffer Stream Memory
    ...
    for idx, row in df_jadual.iterrows():
        ...
        html += f"""
            <tr>
                ...
                <td>{row['Nama Pegawai']}<br>{row['Nama Penolong']}</td>
...
with tab1:
    ...
    data_sampel = [
        ...
        {"Tarikh": "03/09/2026", "Hari": "Khamis", "Syif / Masa": "Biasa (1700 - 0800)", "Nama Pegawai": "INSP ADDLYZAN BIN ABD MANAP", "No. Tel Pegawai": "012-4739701", "Bhg Pegawai": "E2", "Nama Penolong": "KPL/D ABDUL HALIM BIN KAMARUDDIN", "No. Tel Penolong": "013-2425850", "Bhg Penolong": "E7"},
        {"Tarikh": "04/09/2026", "Hari": "Jumaat", "Syif / Masa": "Biasa (1700 - 0800)", "Nama Pegawai": "INSP SHURENDREN A/L JAYARAMAN", "No. Tel Pegawai": "019-2823312", "Bhg Pegawai": "E2", "Nama Penolong": "KPL/D LO YEE TYNG", "No. Tel Penolong": "016-8374537", "Bhg Penolong": "E6"}
    ]
...
with tab2:
    ...
    if not df_rekod.empty:
        ...
        # Pembinaan Fail Word & Format HTML Preview

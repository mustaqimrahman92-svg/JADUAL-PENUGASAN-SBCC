...
# REKOD SEPTEMBER 2026 TEPAT MENGIKUT DOKUMEN PDF (16/09 DIKIRA WE)
REKOD_SEP_2026 = {
    ...
    "INSP KASHMINDEJRIT KAUR A/P CHARLES": {"WD": 0, "WE": 1},  # 16/09 Cuti Umum (WE)
    "INSP FAIZ BIN BASAR": {"WD": 0, "WE": 1},                # 16/09 Cuti Umum (WE)
...
with tab1:
    ...
    if st.button("🚀 Jana Jadual Penugasan", type="primary"):
        ...
        # ----------------------------------------------------
        # FUNGSI PEMILIHAN PEGAWAI (KUNCI KETAT BERTINDIH & CAP)
        # ----------------------------------------------------
        def pilih_pegawai(is_wknd, elak_set, is_jumaat=False):
            ...
            # STEP 3: PENAPISAN GILIRAN LELAKI / WANITA & WE/WD (KUNCI KETAT)
            if aktif_giliran_lelaki:
                ...
                # Gunakan calon_sub HANYA jika baki calon mencukupi (>=2)
                if len(calon_sub) >= 2:
                    calon = calon_sub

...
with tab1:
    ...
    if st.button("🚀 Jana Jadual Penugasan", type="primary"):
        ...
        # Dapatkan rekod bulan sebelum untuk semakan rotation
        ...
        def pilih_pegawai(is_wknd, elak_set, is_jumaat=False):
            ...
            def dapatkan_skor(nama_pegawai):
                ...
                # GILIRAN ROTATION KHAS UNTUK PEGAWAI LELAKI
                if aktif_giliran_lelaki and not is_pegawai_wanita(nama_pegawai):
                    rec_lepas = rekod_bulan_lepas.get(nama_pegawai, {"WD": 0, "WE": 0})
                    if is_wknd and rec_lepas["WE"] > 0:
                        penalti_rotation += 500  # Keutamaan ke WD jika WE bulan lepas > 0
                    elif not is_wknd and rec_lepas["WD"] > 0 and rec_lepas["WE"] == 0:
                        penalti_rotation += 500  # Keutamaan ke WE jika WD sahaja bulan lepas

                if is_wknd:
                    return (tugas_we[nama_pegawai] * 100) + (rec["WE"] * 10) + (tugas_wd[nama_pegawai] + tugas_we[nama_pegawai]) + penalti_rotation
                else:
                    return (tugas_wd[nama_pegawai] * 100) + (rec["WD"] * 10) + (tugas_wd[nama_pegawai] + tugas_we[nama_pegawai]) + penalti_rotation
...
with tab2:
    ...
    if not df_rekod.empty:
        ...
        for idx, p in enumerate(PEGAWAI, start=1):
            ...
            if bulan_cari == 9 and tahun_cari == 2026:

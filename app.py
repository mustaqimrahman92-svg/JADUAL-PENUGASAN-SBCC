...
with tab1:
    ...
    if st.button("🚀 Jana Jadual Penugasan", type="primary"):
        ...
        st.session_state['tugas_wd_temp'] = tugas_wd
        st.session_state['tugas_we_temp'] = tugas_we
    ...
    if 'df_jadual_temp' in st.session_state and st.session_state['bulan_temp'] == bulan and st.session_state['tahun_temp'] == tahun:
        ...
        # ==========================================
        # STATISTIK KEKERAPAN & KEBARANGKALIAN
        # ==========================================
        st.markdown("---")
        st.subheader("📊 Statistik Kekerapan & Kebarangkalian Penugasan")
        
        tugas_wd = st.session_state['tugas_wd_temp']
        tugas_we = st.session_state['tugas_we_temp']
        jumlah_penugasan_keseluruhan = len(df_j)
        ...
        for p in PEGAWAI:
            ...
            if nama in pegawai_dikecualikan:
                stat_data.append({
                    "Nama Pegawai": nama,
                    "Tugas Hari Biasa (WD)": 0,
                    "Tugas Hujung Minggu (WE)": 0,
                    "Jumlah Penugasan": 0,
                    "Kekerapan / Kebarangkalian (%)": "0.00% (Dikecualikan)",
                    "Status": "🚫 Dikecualikan"
                })
            else:
                wd = tugas_wd.get(nama, 0)
                we = tugas_we.get(nama, 0)
                total = wd + we
                kebarangkalian = (total / jumlah_penugasan_keseluruhan * 100) if jumlah_penugasan_keseluruhan > 0 else 0
                
                stat_data.append({
                    "Nama Pegawai": nama,
                    "Tugas Hari Biasa (WD)": wd,
                    "Tugas Hujung Minggu (WE)": we,
                    "Jumlah Penugasan": total,
                    "Kekerapan / Kebarangkalian (%)": f"{kebarangkalian:.2f}%",
                    "Status": "✅ Aktif"
                })

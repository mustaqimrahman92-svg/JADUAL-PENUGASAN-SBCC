...
# Senarai Pegawai Rasmi (31 Pegawai)[span_0](start_span)[span_0](end_span)
PEGAWAI = [
...
# Sidebar 2: Tetapan Cuti Umum / Kelepasan Am
st.sidebar.header("🎉 Tetapan Cuti Umum (Public Holiday)")
tarikh_cuti_umum = st.sidebar.multiselect(
    "Pilih tarikh CUTI UMUM pada bulan ini (secara automatik dikira 2 syif):",
...
if st.button("🚀 Jana Jadual Penugasan", type="primary"):
    ...
    def pilih_pegawai(is_wknd, elak_set):
        ...
        # Utamakan pengagihan seimbang mengikut jenis hari (Weekdays/Weekends & Cuti Umum)
        if is_wknd:
    ...
    for p in PEGAWAI:
        ...
        if nama in pegawai_dikecualikan:
            ringkasan_list.append({"Nama Pegawai": nama, "Hari Biasa": 0, "Hujung Minggu / Cuti Umum": 0, "Jumlah Syif": 0, "Status": "DIKECUALIKAN"})
        else:
            wd = tugas_wd[nama]
            we = tugas_we[nama]
            ringkasan_list.append({"Nama Pegawai": nama, "Hari Biasa": wd, "Hujung Minggu / Cuti Umum": we, "Jumlah Syif": wd + we, "Status": "AKTIF"})

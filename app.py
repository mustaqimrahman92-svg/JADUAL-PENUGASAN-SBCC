...
# ==========================================
# 1. FUNGSI PANGKALAN DATA (SQLITE WITH AUTO-FIX)
# ==========================================
...
def init_db():
    ...
    # 2. Semak dan tambah lajur 'tarikh_disahkan' jika tiada
    c.execute("PRAGMA table_info(rekod_jadual)")
    columns = [column[1] for column in c.fetchall()]
    if 'tarikh_disahkan' not in columns:
        c.execute("ALTER TABLE rekod_jadual ADD COLUMN tarikh_disahkan TEXT")
        
    conn.commit()
    conn.close()

def simpan_jadual_db(bulan, tahun, df_jadual):
    init_db() # Pastikan struktur DB terkini sebelum simpan
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
...
def ambil_jadual_db(bulan, tahun):
    init_db() # Self-healing: Kemas kini struktur DB secara automatik sebelum membaca data
    conn = sqlite3.connect(DB_FILE)
    ...
    try:
        ...
        # Peringkat kedua perlindungan sekiranya berlaku sebarang ralat pembacaan
        df = pd.DataFrame()
    finally:
        conn.close()
...
# ----------------------------------------------------
# FUNGSI LAMPIRAN 'A' (GABUNG PEGAWAI & PENOLONG)
# ----------------------------------------------------
...
# JALANKAN INISIALISASI PANGKALAN DATA SEBAIK SAHAJA APLIKASI DIALANKAN
init_db()
...
with tab2:
    ...
    if not df_rekod.empty:
        st.info(f"Rekod Ditemui. Tarikh Disahkan: {df_rekod['Tarikh Disahkan'].iloc[0] if 'Tarikh Disahkan' in df_rekod.columns else 'Disahkan'}")
        html_code = jana_html_lampiran_a(df_rekod, bulan_cari, tahun_cari)
        
        col_dl1, col_dl2, col_del = st.columns([1, 1, 1])
        with col_dl1:
            st.download_button(
                label="🖨️ Muat Turun Lampiran 'A' (HTML)",
                data=html_code,

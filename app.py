...
# Pemetaan Nama Hari ke Bahasa Melayu
NAMA_HARI_BM = {
    "Monday": "Isnin",
    "Tuesday": "Selasa",
    "Wednesday": "Rabu",
...
def init_db():
    ...
    ''')
    c.execute('''
        CREATE TABLE IF NOT EXISTS draf_jadual (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            bulan INTEGER,
            tahun INTEGER,
            tarikh TEXT,
...
# --- FUNGSI DRAF ---
def simpan_draf_db(bulan, tahun, df_jadual):
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    c.execute("DELETE FROM draf_jadual WHERE bulan = ? AND tahun = ?", (bulan, tahun))
    
    tarikh_sekarang = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    for _, row in df_jadual.iterrows():
        c.execute('''
            INSERT INTO draf_jadual (
...
def ambil_draf_db(bulan, tahun):
    conn = sqlite3.connect(DB_FILE)
    query = """
        SELECT id, tarikh AS Tarikh, hari AS Hari, syif AS 'Syif / Masa', 
               ...
               tarikh_draf AS 'Tarikh Draf' 
        FROM draf_jadual WHERE bulan = ? AND tahun = ?
    """
...
def kemaskini_rekod_tertentu(df_edited, bulan, tahun):
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    for _, row in df_edited.iterrows():
        c.execute('''
            UPDATE rekod_jadual 
            SET tarikh = ?, hari = ?, syif = ?, 
                nama_pegawai = ?, no_tel_pegawai = ?, caw_pegawai = ?, 
                nama_penolong = ?, no_tel_penolong = ?, caw_penolong = ?
            WHERE id = ?
        ''', (
            row['Tarikh'], row['Hari'], row['Syif / Masa'], 
            row['Nama Pegawai'], row['No. Tel Pegawai'], row['Cawangan Pegawai'],
            row['Nama Penolong'], row['No. Tel Penolong'], row['Cawangan Penolong'],
            row['id']
        ))
    conn.commit()
    conn.close()
...
PEGAWAI = [
    {"nama": "INSP ADDLYZAN BIN ABD MANAP", "no_tel": "012-4739701", "caw": "E2"},
    {"nama": "INSP FAIZ BIN BASAR", "no_tel": "017-9457941", "caw": "E2"},
    {"nama": "INSP SABARI BIN BUJANG", "no_tel": "016-7135975", "caw": "E2"},
    {"nama": "INSP KASHMINDEJRIT KAUR A/P CHARLES", "no_tel": "016-6738465", "caw": "E2"},
    {"nama": "INSP SHURENDREN A/L JAYARAMAN", "no_tel": "019-2823312", "caw": "E2"},
    {"nama": "INSP FARAH NADJWA BT MOHD LASA", "no_tel": "013-9195659", "caw": "E2"},
    {"nama": "INSP NORAINUN MUBIN BT ANUAR", "no_tel": "012-7493780", "caw": "E3"},
    {"nama": "INSP MUHAMAD IQRAM BIN MOHAMAD BUKHARI", "no_tel": "013-7044459", "caw": "E3"},
    {"nama": "INSP MOHAMAD IQBAL IBRAHIM BIN DAUD", "no_tel": "012-6467775", "caw": "E4"},
    {"nama": "INSP AMI RUSLAN BIN MOHD RUSLAN", "no_tel": "017-9501506", "caw": "E4"},
    {"nama": "INSP MOHD RAZIF BIN ROSSMAN", "no_tel": "017-9196166", "caw": "E4"},
    {"nama": "INSP AZFIZI BIN AZIZ", "no_tel": "014-8340944", "caw": "E4"},
    {"nama": "INSP MUHAMAD ARIFF BIN ABD RAHIM", "no_tel": "010-4017822", "caw": "E4"},
    {"nama": "INSP MOHD RUSDI BIN HASSAN", "no_tel": "010-4535060", "caw": "E4"},
    {"nama": "INSP ABDUL MUIN BIN AB AZIZ", "no_tel": "019-4494500", "caw": "E5"},
    {"nama": "INSP MOHD SHAHRIL BIN MOHAMED RESALI", "no_tel": "012-9096486", "caw": "E5"},
    {"nama": "INSP NOR HAIZAH BINTI YACOB", "no_tel": "012-9979790", "caw": "E5"},
    {"nama": "INSP KUKENDRAN A/L YOGENDRAN", "no_tel": "012-9062264", "caw": "E6"},
    {"nama": "INSP MUHAMMAD NOR QHAIREIL BIN NOR AZMI", "no_tel": "011-16554250", "caw": "E6"},
    {"nama": "INSP KHAIRUL HAFIZ BIN SHAFFIE AHMED", "no_tel": "017-4308434", "caw": "E6"},
    {"nama": "INSP EZYANTI BT MUHAMAD", "no_tel": "019-9241360", "caw": "E6"},
    {"nama": "INSP MOHAMAD SYARUL FADLI BIN MAT HUSAIN", "no_tel": "011-10042323", "caw": "E6"},
    {"nama": "INSP MUHAMMAD AFIQ BIN NORDIN", "no_tel": "010-4641637", "caw": "E7"},
    {"nama": "INSP MOHD JEFFERE BIN ALI", "no_tel": "014-2348186", "caw": "E7"},
    {"nama": "INSP NUR AADILA BINTI ABDUL MALEK", "no_tel": "014-3383604", "caw": "E7"},
    {"nama": "INSP MOHD SHAHRILBUN BIN AMDAN", "no_tel": "017-5800578", "caw": "E8"},
    {"nama": "INSP MOHD FAIZ BIN ESA", "no_tel": "010-7978603", "caw": "E8"},
    {"nama": "INSP ROHAIZASHAFIKA BINTI MOHAMAD RADZALI", "no_tel": "016-2448782", "caw": "E8"},
    {"nama": "INSP DZUL FADHLI BIN ABDUL HALIM", "no_tel": "014-6689803", "caw": "E9"},
    {"nama": "INSP NORHAZIERAM BINTI ZAKARIA", "no_tel": "014-9207312", "caw": "E9"},
    {"nama": "INSP WAN MUHAMAD MUSTAQIM BIN ABDUL RAHMAN", "no_tel": "012-9212972", "caw": "E9"}
]

PENOLONG_PEGAWAI = [
    {"nama": "SJN/D ONG WEI TAU", "no_tel": "012-8121981", "caw": "E2"},
    {"nama": "KPL/D MOHD FAIZAL BIN TIMAN", "no_tel": "017-7455348", "caw": "E2"},
    {"nama": "KPL/D MUHAMMAD AZRUL SHAFIQ BIN ISMAN", "no_tel": "019-5898658", "caw": "E2"},
    {"nama": "KPL/D FARIEZUL NAIM BIN MOHD SANI", "no_tel": "013-6399275", "caw": "E2"},
    {"nama": "SJN/D SUYANI BINTI MAT SAAD", "no_tel": "012-3405606", "caw": "E2"},
    {"nama": "SJN/D MOHD JURAIDI BIN SAMSUDIN", "no_tel": "012-6769745", "caw": "E2"},
    {"nama": "L/KPL/D NURHIDAYAH BINTI SUHAMI", "no_tel": "017-7011952", "caw": "E2"},
    {"nama": "KONS NUR AMIERA BINTI MOHAMAD ALI AHAN", "no_tel": "017-6723075", "caw": "E2"},
    {"nama": "L/KPL ATHIRAH NABILA BINTI SAEDIN", "no_tel": "018-3166724", "caw": "E2"},
    ...
    {"nama": "L/KPL AHMAD HELMI HARIRI BIN SALVEHUJRAIE", "no_tel": "011-166522645", "caw": "E3"},
...
# ==========================================
tab1, tab2, tab3 = st.tabs(["⚙️ Penjanaan & Draf Jadual", "📜 Rekod Sejarah & Format Cetak", "✏️ Edit & Kemaskini Rekod"])
...
with tab1:
    ...
    df_draf_ada = ambil_draf_db(bulan, tahun)
    if not df_draf_ada.empty and 'df_jadual_temp' not in st.session_state:
        st.session_state['df_jadual_temp'] = df_draf_ada
        st.session_state['bulan_temp'] = bulan
        st.session_state['tahun_temp'] = tahun
        st.info(f"📂 **Draf Ditemui:** Draf jadual Bulan {bulan}/{tahun} yang tersimpan telah dimuatkan.")
    ...
    tarikh_cuti_umum = st.sidebar.multiselect(
        ...
        format_func=lambda d: f"{d.strftime('%d/%m/%Y')} ({NAMA_HARI_BM.get(d.strftime('%A'), d.strftime('%A'))})"
    )
    ...
    # ==========================================
    # MODUL SANDARAN PANGKALAN DATA (BACKUP & RESTORE)
    # ==========================================
    st.sidebar.markdown("---")
    st.sidebar.subheader("💾 Sandaran Pangkalan Data")

    # 1. Muat Turun Fail Database (.db)
    try:
        with open(DB_FILE, "rb") as fp:
            st.sidebar.download_button(
                label="📥 Muat Turun Backup Database",
                data=fp,
                file_name="jadual_penugasan.db",
                mime="application/x-sqlite3"
            )
    except FileNotFoundError:
        pass

    # 2. Muat Naik Semula Fail Database (.db) jika server reboot
    uploaded_db = st.sidebar.file_uploader("📤 Restore Database (.db)", type=["db"])
    if uploaded_db is not None:
        with open(DB_FILE, "wb") as f:
            f.write(uploaded_db.getbuffer())
        st.sidebar.success("✅ Pangkalan data berjaya dipulihkan!")
        st.rerun()

    if st.button("🚀 Jana Jadual Penugasan Baru", type="primary"):
        ...
        for dt in senarai_tarikh:
            ...
            nama_hari_eng = dt.strftime("%A")
            nama_hari_bm = NAMA_HARI_BM.get(nama_hari_eng, nama_hari_eng)
            hari_str = nama_hari_bm + (" (CUTI UMUM)" if dt in tarikh_cuti_umum else "")
    ...
    # PAPARAN & SUNTINGAN DRAF DENGAN DROP-DOWN LIST
    if 'df_jadual_temp' in st.session_state and st.session_state['bulan_temp'] == bulan and st.session_state['tahun_temp'] == tahun:
        st.subheader(f"📅 Paparan / Suntingan Draf Penugasan (Bulan {bulan}/{tahun})")
        st.info("💡 **Petua Edit:** Klik petak **Nama Pegawai** atau **Nama Penolong** untuk memilih nama dari senarai *drop-down*.")

        senarai_nama_pegawai = [p["nama"] for p in PEGAWAI]
        senarai_nama_penolong = [p["nama"] for p in PENOLONG_PEGAWAI]

        df_edit_draf = st.data_editor(
            st.session_state['df_jadual_temp'],
            column_config={
                "Nama Pegawai": st.column_config.SelectboxColumn(
                    "Nama Pegawai (Insp)",
                    options=senarai_nama_pegawai,
                    required=True
                ),
                "Nama Penolong": st.column_config.SelectboxColumn(
                    "Nama Penolong Pegawai",
                    options=senarai_nama_penolong,
                    required=True
                )
            },
            use_container_width=True,
            num_rows="fixed",
            key="draf_editor_select"
        )

        for idx, row in df_edit_draf.iterrows():
            peg_obj = next((p for p in PEGAWAI if p["nama"] == row["Nama Pegawai"]), None)
            if peg_obj:
                df_edit_draf.at[idx, "No. Tel Pegawai"] = peg_obj["no_tel"]
                df_edit_draf.at[idx, "Cawangan Pegawai"] = peg_obj["caw"]

            pen_obj = next((p for p in PENOLONG_PEGAWAI if p["nama"] == row["Nama Penolong"]), None)
            if pen_obj:
                df_edit_draf.at[idx, "No. Tel Penolong"] = pen_obj["no_tel"]
                df_edit_draf.at[idx, "Cawangan Penolong"] = pen_obj["caw"]

        st.session_state['df_jadual_temp'] = df_edit_draf

        col_btn1, col_btn2 = st.columns(2)
        with col_btn1:
            if st.button("💾 Simpan Sebagai Draf", type="primary", use_container_width=True):
                simpan_draf_db(bulan, tahun, df_edit_draf)
                st.success(f"✅ Draf bagi Bulan {bulan}/{tahun} BERHASIL DISIMPAN!")
        with col_btn2:
            if st.button("🔒 Sahkan & Simpan Rasmi", type="secondary", use_container_width=True):
                simpan_jadual_db(bulan, tahun, df_edit_draf)
                st.success(f"🎉 Jadual Rasmi bagi Bulan {bulan}/{tahun} BERHASIL DISAHKAN & DISIMPAN!")
...
with tab2:
    ...
    if not df_rekod.empty:
        st.info(f"Rekod Rasmi Ditemui. Tarikh Disahkan: {df_rekod['Tarikh Disahkan'].iloc[0]}")
        ...
        st.warning(f"Tiada rekod jadual rasmi yang disahkan ditemui bagi Bulan {bulan_cari}/{tahun_cari}.")

# ==========================================
# TAB 3: EDIT & KEMASKINI REKOD RASMI
# ==========================================
with tab3:
    st.subheader("✏️ Edit & Kemaskini Rekod Rasmi Pangkalan Data")
    col_e1, col_e2 = st.columns(2)
    with col_e1:
        bulan_edit = st.selectbox("Pilih Bulan Untuk Di-edit", list(range(1, 13)), index=8, key="b_edit")
    with col_e2:
        tahun_edit = st.number_input("Pilih Tahun Untuk Di-edit", min_value=2024, max_value=2030, value=2026, key="t_edit")

    df_edit = ambil_jadual_db(bulan_edit, tahun_edit)

    if not df_edit.empty:
        st.info("💡 Anda boleh menukar terus maklumat dalam jadual rasmi ini. Tekan **'💾 Simpan Perubahan'** setelah selesai.")
        
        senarai_nama_pegawai = [p["nama"] for p in PEGAWAI]
        senarai_nama_penolong = [p["nama"] for p in PENOLONG_PEGAWAI]

        df_kemaskini = st.data_editor(
            df_edit,
            column_config={
                "Nama Pegawai": st.column_config.SelectboxColumn(
                    "Nama Pegawai (Insp)",
            ...
            disabled=["id", "Tarikh Disahkan"],
            use_container_width=True,
            num_rows="fixed"
        )

        for idx, row in df_kemaskini.iterrows():
            peg_obj = next((p for p in PEGAWAI if p["nama"] == row["Nama Pegawai"]), None)
            if peg_obj:
                df_kemaskini.at[idx, "No. Tel Pegawai"] = peg_obj["no_tel"]
                df_kemaskini.at[idx, "Cawangan Pegawai"] = peg_obj["caw"]

            pen_obj = next((p for p in PENOLONG_PEGAWAI if p["nama"] == row["Nama Penolong"]), None)
            if pen_obj:
                df_kemaskini.at[idx, "No. Tel Penolong"] = pen_obj["no_tel"]
                df_kemaskini.at[idx, "Cawangan Penolong"] = pen_obj["caw"]

        if st.button("💾 Simpan Perubahan Rekod Rasmi", type="primary"):
            kemaskini_rekod_tertentu(df_kemaskini, bulan_edit, tahun_edit)
            st.success(f"✅ Rekod rasmi bagi Bulan {bulan_edit}/{tahun_edit} berjaya dikemaskini!")
            st.rerun()
    else:
        st.warning(f"Tiada rekod jadual rasmi ditemui bagi Bulan {bulan_edit}/{tahun_edit} untuk di-edit.")

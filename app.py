...
def ambil_semua_rekod_db():
    conn = sqlite3.connect(DB_FILE)
    query = "SELECT bulan, tahun, tarikh, hari, syif, nama_pegawai FROM rekod_jadual"
...
# Rekod Sebenar September 2026 sebagai Pedoman Rasmi
REKOD_SEP_2026 = {
...
with tab1:
    ...
    if st.button("🚀 Jana Jadual Penugasan", type="primary"):
        ...
        def pilih_pegawai(is_wknd, elak_set, is_jumaat=False):
            ...
            # ELAK PEGAWAI JUMAAT DARI MENDAPAT SABTU/AHAD DALAM BULAN YANG SAMA
            if is_wknd:
        ...
        for dt in senarai_tarikh:
            ...
            if is_weekend:
                ...
                p_b = override_dict[k_b] if k_b in override_dict else pilih_pegawai(False, pegawai_hari_sebelumnya, is_jumaat=is_jumaat)
    ...
    if 'df_jadual_temp' in st.session_state and st.session_state['bulan_temp'] == bulan and st.session_state['tahun_temp'] == tahun:
        ...
        st.markdown("---")
        st.subheader("📊 Rekod Statistik Kekerapan Penugasan (Bermula Sep 2026)")
        
        stat_list = []
        df_semua_db = ambil_semua_rekod_db()
        ...
        for idx, p in enumerate(PEGAWAI, start=1):
            ...
            wd_db = 0
            we_db = 0
            if not df_semua_db.empty:
                df_p = df_semua_db[df_semua_db['nama_pegawai'] == nama_p]
                for _, r in df_p.iterrows():
                    if r['tahun'] > 2026 or (r['tahun'] == 2026 and r['bulan'] > 9):
                        is_wk = ("Siang" in r['syif'] or "Malam" in r['syif'] or "CUTI UMUM" in r['hari'])
                        if is_wk:
                            we_db += 1
                        else:
                            wd_db += 1
            ...
            wd_semasa = 0
            we_semasa = 0
            if bulan > 9 or tahun > 2026:
                df_p_semasa = df_j[df_j['Nama Pegawai'] == nama_p]
                for _, r in df_p_semasa.iterrows():
            ...
            tot_wd = wd_sep + wd_db + wd_semasa
            tot_we = we_sep + we_db + we_semasa
            tot_keseluruhan = tot_wd + tot_we

            stat_list.append({
                "BIL": f"{idx:02d}.",
                "NAMA PEGAWAI": nama_p,
                "WEEKDAY (HARI BIASA)": tot_wd,
                "WEEKEND / CUTI UMUM": tot_we,
                "JUMLAH KEKERAPAN TUGAS": tot_keseluruhan
            })
